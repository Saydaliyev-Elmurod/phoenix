...
public class CustomRepository {
    ...
    public List<ProductStatisticHelper> getProductStatistic(Instant dateFrom, Instant dateTo) {
        ...
        String query = "select " +
                ...
                " sum(oi.price*oi.count) as totalPrice " +
                " from " + Constants.SCHEMA + "." + Constants.TABLE_ORDER_PRODUCT + " oi" +
                " inner join " + Constants.SCHEMA + "." + Constants.TABLE_BOOK + " p on oi.book_id = p.id" +
                " where oi.deleted = false ";

        if (dateFrom != null) {
            query += "and oi.created_date >= :dateFrom ";
            parameters.put("dateFrom", Timestamp.from(dateFrom));
        }
        if (dateTo != null) {
            query += "and oi.created_date <= :dateTo ";
            parameters.put("dateTo", Timestamp.from(dateTo));
        }
        ...
        return namedParameterJdbcTemplate.query(
                query, parameters, new BeanPropertyRowMapper<>(ProductStatisticHelper.class));
    }
}
